...
export default function TechnicalVerificationDashboard() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Verificare Tehnică</h2>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Analiză tehnică a documentațiilor transmise de la verificare inițială.</p>
      </div>
                ...
                {items.map((req) => (
                    <Card key={req.id} className="hover:shadow-md transition-shadow dark:bg-slate-900 dark:border-slate-800">
                        <CardHeader className="flex flex-col lg:flex-row lg:items-center justify-between pb-2 gap-4">
                            ...
                            {activeTab === 'queue' && (
                                <Button size="sm" onClick={() => handlePickUp(req.id)} className="bg-orange-600 hover:bg-orange-700 text-white">
