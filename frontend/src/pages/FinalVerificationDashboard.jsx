...
export default function FinalVerificationDashboard() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-slate-800">Verificare Finală</h2>
        <p className="text-slate-500 mt-1">Aprobarea finală a documentelor și emiterea deciziei.</p>
      </div>
                ...
                {items.map((req) => (
                    <Card key={req.id} className="hover:shadow-md transition-shadow">
                        <CardHeader className="flex flex-col lg:flex-row lg:items-center justify-between pb-2 gap-4">
                            ...
                            {activeTab === 'queue' && (
                                <Button size="sm" onClick={() => handlePickUp(req.id)} className="bg-purple-600 hover:bg-purple-700">
