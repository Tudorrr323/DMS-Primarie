...
export default function RequestCard({ request }) {
  return (
    <Link to={`/requests/${request.id}`} className="block">
      <Card className="flex flex-col h-full transition-all hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600">
        <CardHeader>
          <CardTitle className="text-lg truncate">{request.title}</CardTitle>
        </CardHeader>
        <CardContent className="flex-grow">
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{request.category}</p>
        </CardContent>
