...
-- Note: PostgreSQL doesn't support CREATE TYPE IF NOT EXISTS directly.
-- Using a DO block to ensure re-runnability.
do $$ 
begin
...
-- User Space
drop policy if exists "Owner space visibility" on vfs.user_space;
...
for select using (
  (space_id in (select id from vfs.user_space where profile_id = (select auth.uid())) 
   or vfs.has_access(id, (select auth.uid())))
  and deleted_at is null
);

drop policy if exists "View trash" on vfs.folders;
...
drop policy if exists "Owner manage folders" on vfs.folders;
...
drop policy if exists "View trash files" on vfs.files;
...
drop policy if exists "Owner manage files" on vfs.files;
...
-- Versions
drop policy if exists "Version access" on vfs.file_versions;
create policy "Version access" on vfs.file_versions
for select using (exists (select 1 from vfs.files where id = file_id));

drop policy if exists "Owner manage versions" on vfs.file_versions;
...
for all using (
  exists (
    select 1 from vfs.folders where id = folder_id 
    and space_id in (select id from vfs.user_space where profile_id = (select auth.uid()))
  ) or 
  exists (
    select 1 from vfs.files where id = file_id 
    and space_id in (select id from vfs.user_space where profile_id = (select auth.uid()))
  )
);

drop policy if exists "Read own permissions" on vfs.permissions;
create policy "Read own permissions" on vfs.permissions
for select using (profile_id = (select auth.uid()));
...
begin
    ...
    if target_space_id is null and tg_table_name = 'permissions' then
        ...
        if target_space_id is null and old.folder_id is not null then
    ...
    final_action := case 
        ...
        when tg_op = 'UPDATE' then 
            case 
                when old.parent_id is distinct from new.parent_id then 'MOVE'::vfs.audit_action_type
                when old.deleted_at is not null and new.deleted_at is null then 'RESTORE'::vfs.audit_action_type
                when old.deleted_at is null and new.deleted_at is not null then 'DELETE'::vfs.audit_action_type
    ...
    if target_space_id is not null then
        ...
        values (target_space_id, current_user_id, final_entity, coalesce(new.id, old.id), final_action, 
...
drop policy if exists "VFS Access" on storage.objects;
...
for select using (
  ...
  and exists (
    ...
    and (
       f.space_id in (select id from vfs.user_space where profile_id = (select auth.uid()))
       or exists (select 1 from vfs.permissions p where p.file_id = f.id and p.profile_id = (select auth.uid()))
       or (f.folder_id is not null and vfs.has_access(f.folder_id, (select auth.uid())))
    )
  )
);

drop policy if exists "VFS Upload" on storage.objects;
...
drop policy if exists "VFS Delete" on storage.objects;
