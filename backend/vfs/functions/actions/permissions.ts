...
import { Permission, AccessLevel } from '../types';

/**
 * Fetches permissions for a specific folder or file.
 */
export async function getPermissions(itemId: string, itemType: 'folder' | 'file'): Promise<Permission[]> {
  try {
    const column = itemType === 'folder' ? 'folder_id' : 'file_id';
    
    // Join with public.profiles (if available) to get user details
    // Note: Cross-schema joins work in Supabase if permissions allow.
    // Assuming 'public.profiles' is readable.
    ...
    // Map the response to handle the 'user' relation which might be returned as an array
    const permissions = (response.data as any[]).map(p => ({
      id: p.id,
